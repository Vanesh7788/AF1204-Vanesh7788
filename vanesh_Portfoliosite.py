# /// script
# requires-python = ">=3.13"
# dependencies = [
#     "marimo>=0.19.10",
#     "pandas>=2.3.3",
#     "plotly>=6.5.1",
#     "pyarrow>=22.0.0",
#     "pyzmq>=27.1.0",
# ]
# ///

import marimo

__generated_with = "0.19.11"
app = marimo.App()


@app.cell
def _(mo):
    mo.md(r"""
    ---
    ## AF1204 Individual Portfolio  |  Vanesh Kumar
    BSc Accounting and Finance  |  City St George's, University of London
    """)
    return


@app.cell
def _():
    import marimo as mo
    import pandas as pd
    import micropip
    return micropip, mo, pd


@app.cell
def _(pd):
    # S&P 500 dataset loaded from lecturer gist — WASM-safe remote URL (Week 4)
    _URL = (
        "https://gist.githubusercontent.com/DrAYim/"
        "80393243abdbb4bfe3b45fef58e8d3c8/raw/"
        "ed5cfd9f210bf80cb59a5f420bf8f2b88a9c2dcd/"
        "sp500_ZScore_AvgCostofDebt.csv"
    )
    vk_df = pd.read_csv(_URL)

    # Drop rows with missing values in key columns (Week 2 — exception handling)
    vk_df = vk_df.dropna(subset=["AvgCost_of_Debt", "Z_Score_lag", "Sector_Key",
                                   "Market_Cap", "Name", "Ticker"])

    # Winsorise — remove extreme outliers (Week 6 — data cleaning)
    vk_df = vk_df[vk_df["AvgCost_of_Debt"] < 5]
    vk_df = vk_df[vk_df["Z_Score_lag"].between(-10, 40)]

    # Feature engineering (Week 4)
    vk_df = vk_df.copy()
    vk_df["cost_pct"] = vk_df["AvgCost_of_Debt"] * 100
    vk_df["mcap_bn"]  = vk_df["Market_Cap"] / 1e9

    # Pre-aggregate sector statistics — used by multiple tabs
    sec_stats = (
        vk_df.groupby("Sector_Key")
        .agg(
            avg_cost_pct=("cost_pct", "mean"),
            avg_zscore=("Z_Score_lag", "mean"),
            avg_mcap_bn=("mcap_bn", "mean"),
            n_companies=("Name", "nunique"),
            n_obs=("Name", "count"),
        )
        .round(3)
        .reset_index()
        .rename(columns={"Sector_Key": "Sector"})
    )

    # Top 5 companies per sector by average market cap — for sunburst chart
    _tc = (
        vk_df.groupby(["Sector_Key", "Name"])["mcap_bn"]
        .mean()
        .reset_index()
    )
    _tc = _tc[_tc["mcap_bn"] > 0]
    _tc = _tc.sort_values("mcap_bn", ascending=False)
    _tc = (
        _tc.groupby("Sector_Key", group_keys=False)
        .apply(lambda g: g.head(5))
        .reset_index(drop=True)
    )
    top_companies = _tc.rename(columns={
        "Sector_Key": "Sector",
        "Name": "Company",
        "mcap_bn": "Avg_MCap_Bn",
    })

    return sec_stats, top_companies, vk_df


@app.cell
async def _(micropip):
    await micropip.install("plotly");
    import plotly.express as px
    return (px,)


@app.cell
def _(mo):
    import base64
    _raw = base64.b64decode("")
    cv_btn = mo.download(
        data=_raw,
        filename="Vanesh_Kumar_CV.docx",
        mimetype="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        label="Download CV",
    )
    return (base64, cv_btn)


@app.cell
def _(cv_btn, mo):
    _left = mo.md("""
**Education**

BSc Accounting and Finance
City St Georges, University of London | 2025 to 2028

International Foundation Year (Computer Science)
Durham University | Completed August 2025

A-Levels: Computer Science, Mathematics, Physics
Foundation Public School, Hyderabad | 2022 to 2024

GCSEs / O-Levels: 8 subjects including Mathematics and English Language

---

**Experience**

Accounting Intern, United Agro Chemicals | Oct 2024 to Nov 2024
- Organised financial documents, checked invoices, updated records in Excel
- Developed practical understanding of business accounting workflows

Finance Accounting Intern, SGM Sugar Mills Ltd | Jun 2024 to Jul 2024
- Assisted in maintaining ledgers and reviewing expenses for accuracy
- Gained exposure to cost structures and basic financial reporting
    """)

    _right = mo.vstack([
        mo.md("**Download CV**"),
        cv_btn,
        mo.md("""
---

**Technical Skills**

Python, pandas, Plotly, Marimo, numpy
GitHub, GitHub Pages, WASM deployment
Excel (formulas, data handling)
LLM prompt engineering, RAG

---

**Professional Skills**

Financial documentation and ledger management
Invoice verification and expense review
Data cleaning and exploratory analysis
Attention to detail in numerical work

---

**Languages**

English, Urdu

---

**Interests**

Financial markets and equity analysis
Entrepreneurship and business strategy
Independent study in accounting and finance
        """),
    ])

    tab_profile = mo.hstack([_left, _right], gap=4, widths=[2, 1])
    return (tab_profile,)


@app.cell
def _(mo, px, sec_stats):
    # Grouped bar: cost of debt vs Z-Score per sector — Week 3/4 visualisation
    _melt = sec_stats[["Sector", "avg_cost_pct", "avg_zscore"]].melt(
        id_vars="Sector",
        value_vars=["avg_cost_pct", "avg_zscore"],
        var_name="Metric",
        value_name="Value",
    )
    _melt["Metric"] = _melt["Metric"].map({
        "avg_cost_pct": "Avg Cost of Debt (%)",
        "avg_zscore": "Avg Altman Z-Score",
    })

    fig_bar = px.bar(
        _melt,
        x="Value",
        y="Sector",
        color="Metric",
        barmode="group",
        orientation="h",
        title="Average Cost of Debt vs Altman Z-Score by Sector",
        labels={"Value": "Metric Value", "Sector": ""},
        template="plotly_white",
        width=880,
        height=500,
        color_discrete_map={
            "Avg Cost of Debt (%)": "#2c7bb6",
            "Avg Altman Z-Score":   "#d7191c",
        },
    )
    fig_bar.update_layout(legend=dict(orientation="h", y=1.08))
    chart_bar = mo.ui.plotly(fig_bar)

    # Summary table embedded in dashboard
    stats_display = sec_stats.rename(columns={
        "avg_cost_pct": "Mean Cost of Debt (%)",
        "avg_zscore":   "Mean Z-Score",
        "avg_mcap_bn":  "Mean MCap (Bn)",
        "n_companies":  "Companies",
        "n_obs":        "Observations",
    })
    stats_tbl = mo.ui.table(stats_display)

    total_obs = int(sec_stats["n_obs"].sum())
    total_cos = int(sec_stats["n_companies"].sum())

    # Dashboard-style layout using mo.md() embedding (Week 4 dashboard technique)
    tab_sector = mo.md(f"""
## Sector Dashboard

A cross-sector view of S&P 500 financial health metrics, aggregated
across all available company-year observations in the dataset.

---

### Summary

| Total Observations | Unique Companies | Sectors Covered |
|:-:|:-:|:-:|
| **{total_obs}** | **{total_cos}** | **{len(sec_stats)}** |

---

### Sector Statistics Table

{stats_tbl}

---

### Grouped Bar Chart: Cost of Debt and Z-Score by Sector

{chart_bar}

---

**Skills demonstrated:**
- pandas groupby aggregation with multiple metrics — Week 4
- px.bar horizontal grouped bar chart — Week 3 advanced visualisation
- Dashboard layout using mo.md() f-string embedding — Week 4 dashboard template
- Cross-sector financial comparison using Z-Score and cost of debt — Week 2
    """)
    return (tab_sector,)


@app.cell
def _(mo, tab_profile, tab_sector):
    app_tabs = mo.ui.tabs({
        "Profile":          tab_profile,
        "Sector Dashboard": tab_sector,
    })
    mo.md(f"""
# Vanesh Kumar
AF1204 Individual Portfolio | BSc Accounting and Finance | City St Georges, University of London

---

{app_tabs}
    """)
    return


if __name__ == "__main__":
    app.run()
